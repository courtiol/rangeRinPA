...
data_rangers %>%
  ggplot() +
  aes(y = staff_rangers, x = staff_others, label = countryname) +
...
data_rangers %>%
  ggplot() +
  aes(y = area_PA_WDPA, x = area_PA_surveyed, label = countryname) +
  geom_text() +
...
data_rangers %>%
  ggplot() +
  aes(y = area_PA_WDPA, x = area_PA_total, label = countryname) +
  geom_text() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") + 
  coord_trans(x = "log1p", y = "log1p") +
  scale_x_continuous(breaks = c(0, 10, 100, 1000, 1e4, 1e5, 1e6, 1e7), minor_breaks = NULL) +
...
data_rangers %>%
  ggplot() +
  aes(y = staff_rangers, x = area_PA_WDPA, label = countryname) +
  geom_text() +
...
data_rangers %>%
  ggplot() +
  aes(y = area_country, x = area_PA_WDPA, label = countryname) +
  geom_text() +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") + 
  coord_trans(x = "log1p", y = "log1p") +
  scale_x_continuous(breaks = c(0, 10, 100, 1000, 1e4, 1e5, 1e6, 1e7), minor_breaks = NULL) +
...
data_rangers %>%
  ggplot() +
  aes(y = staff_rangers, x = area_country, label = countryname) +
  geom_text() +
  coord_trans(x = "log", y = "log1p") +
  stat_function(fun = f, colour = "blue", n = 1e5) +
  scale_x_continuous(breaks = c(10, 100, 1000, 1e4, 1e5, 1e6, 1e7), minor_breaks = NULL, limits = c(10, 1e7)) +
...
data_rangers %>%
  filter(area_country > 1) %>%
    ggplot() +
    aes(y = staff_rangers, x = area_PA_WDPA/area_country, label = countryname, size = area_country) +
    geom_text() +
    coord_trans(y = "log1p") +
    scale_y_continuous(breaks = c(0, 10, 100, 1000, 1e4, 1e5, 1e6, 1e7), minor_breaks = NULL) +
...
data_rangers %>%
    ggplot() +
    aes(y = staff_rangers, x = reliability, label = countryname, colour = country_UN_continent) +
...
data_rangers %>%
  ggplot() +
  aes(y = staff_rangers, x = area_forest_pct, label = countryname, colour = country_UN_continent) +
  geom_text() +
  coord_trans(y = "log1p") +
  scale_y_continuous(breaks = c(0, 10, 100, 1000, 1e4, 1e5, 1e6, 1e7), minor_breaks = NULL) +
  theme_minimal() +
...
### with EVI, SPI, and EPI

We start by comparing the different ecological indexes together:

```{r EVI_SPI_EPI plot, fig.height=8, fig.width=8, cache=TRUE}
GGally::ggpairs(data_rangers[, c("EVI", "SPI", "EPI_2020")]) +
  theme_minimal()
```

**Conclusion**: all 3 indicators are quite correlated between each others.

```{r corr EVI_SPI_EPI plot, fig.height=24, fig.width=8, cache=TRUE}
data_rangers %>%
  pivot_longer(c("EVI", "SPI", "EPI_2020"), names_to = "indicator") %>%
  ggplot() +
  aes(y = staff_rangers, x = value, label = countryname, colour = indicator) +
  geom_text() +
