...
```{r lm}
fit_allometry <- lm(log(staff_rangers + 1) ~  log(PA_area), data = data_rangers_noG)
...
```{r lm2}
fit_allometry2 <- lm(log(staff_rangers + 1) ~  log(PA_area) - 1, data = data_rangers_noG)
...
```{r lm3}
fit_allometry3 <- lm(log(staff_rangers + 1)/log(PA_area) ~ 1, data = data_rangers_noG)
...
```{r comparing models}
fit_model2 <- fitme(I(log(staff_rangers + 1)/log(PA_area)) ~ 1, data = data_rangers_noG)
fit_model3 <- fitme(I(log(staff_rangers + 1)/log(PA_area)) ~ 1, prior.weights = sqrt(1/log(PA_area)), data = data_rangers_noG)
```
...
data_rangers_noG %>%
  mutate(predicted_rangers = data_rangers_noG$PA_area^coef(fit_allometry2)[[1]] - 1,
...
```{r simu5, cache=TRUE}
fit_allometry5_spaMM <- fitme(log(staff_rangers + 1)  ~ log(PA_area) - 1, data = data_rangers_noG)
...
```{r lm pop density}
fit_pop <- lm(log(staff_rangers + 1) ~ log(PA_area) + log(pop_density), data = data_rangers_noG)
...
refit_model2 <- lm(log(staff_rangers + 1) ~ log(PA_area) - 1, data = data_rangers_noG[!is.na(data_rangers_noG$pop_density), ])
```
...
fit_allometry6_spaMM <- fitme(log(staff_rangers + 1) ~ log(PA_area) + log(pop_density), data = data_rangers_noG)
...
- the predictions for the number of rangers from the model accounting for PA_area and spatial autocorrelation for the countries for which we have no population density but lat and long: `r data_rangers %>% filter(is.na(staff_rangers), is.na(pop_density), !is.na(lat)) %>% pull(flag)`
- the predictions for the number of rangers from the model accounting for PA_area and not considering spatial autocorrelation for the countries for which we have no population density, nor lat or long: `r data_rangers %>% filter(is.na(staff_rangers), is.na(pop_density), is.na(lat)) %>% pull(flag)`
- the predictions for the number of rangers from the model accounting for PA_area, pop_density and spatial autocorrelation for the countries for which we do have population density and lat and long: `r data_rangers %>% filter(is.na(staff_rangers), !is.na(pop_density), !is.na(lat)) %>% pull(flag)`
...
fit_matern_pop_noarea_GDP <- fitme(log(staff_rangers + 1) ~ 1 + log(PA_area) + log(pop_density) + log(GDP_2019) + Matern(1|long + lat),
                            data = d, control.dist = list(dist.method = "Earth"))
fit_matern_pop_noarea_noGDP <- fitme(log(staff_rangers + 1) ~ 1 + log(PA_area) + log(pop_density) + Matern(1|long + lat),
                            data = d, control.dist = list(dist.method = "Earth"))
...
fit_matern_pop_area_GDP <- fitme(log(staff_rangers + 1) ~ 1 + log(PA_area) + log(pop_density) + log(GDP_2019) + log(area_country) + Matern(1|long + lat),
                                 data = d, control.dist = list(dist.method = "Earth"))
fit_matern_pop_area_noGDP <- fitme(log(staff_rangers + 1) ~ 1 + log(PA_area) + log(pop_density) + log(area_country) + Matern(1|long + lat),
                                   data = d, control.dist = list(dist.method = "Earth"))
...
fit_no_pop_size_no_coord_area <- fitme(log(staff_rangers + 1)  ~ 0 +  log(PA_area) + log(area_country), data = data_rangers_noG)

fit_matern_no_pop_area <- fitme(log(staff_rangers + 1) ~ 1 + log(PA_area) + log(area_country) + Matern(1|long + lat),
                                data = data_for_matern_pop,
                                control.dist = list(dist.method = "Earth"))

fit_matern_pop_area <- fitme(log(staff_rangers + 1) ~ 1 + log(PA_area) + log(pop_density) + log(area_country),
                             data = data_for_matern_pop,
                             control.dist = list(dist.method = "Earth"))
```
