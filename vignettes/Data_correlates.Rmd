...
data_rangers %>%
  ggplot() +
  aes(y = area_PA_WDPA, x = PA_area_surveyed, label = countryname, size = area_country, colour = country_UN_continent) +
  #geom_text_repel(nudge_x = 0, nudge_y = 0, force = 0.03, force_pull = 2, seed = 123, min.segment.length = 0) +
  geom_text() +
...
```{r corr area_PA_total plot, fig.height=8, fig.width=8, cache=TRUE}
data_rangers %>%
  ggplot() +
...
```{r corr area_PA_WDPA_log plot, fig.height=8, fig.width=8, cache=TRUE}
data_rangers %>%
  ggplot() +
