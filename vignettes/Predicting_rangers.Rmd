...
data_rangers %>%
  group_by(countryname_iso) %>%
  slice(1) %>% ## to remove one duplicate for French polynesia
  ungroup() %>%
...
We can then do the same to predict the sum of rangers across the countries we did not observe:
...
We thus have an uncertainty of around `r round(quantile(simu2, probs = 0.975)[[1]] - quantile(simu2, probs = 0.025)[[1]])` rangers. Notice however that the interval for our prediction is not centred around the mean prediction, which implies that there is less uncertainty about the lowest possible number of rangers than about the highest possible one. Indeed, 50% of the simulation give a number of unknown rangers between 0 and `r round(quantile(simu2, probs = 0.5)[[1]])` while there is a long tail on the right of the distribution.
