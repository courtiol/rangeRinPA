...
Another way to simulate the non-observed number of rangers is to use the linear model we fitted to simulate new data.
...
```{r simu3, cache=TRUE}
fit_allometry2_spaMM <- fitme(log(staff_rangers + 1) ~  log(PA_area) - 1, data = data_rangers_clean_noG)
set.seed(1)
simu3 <- replicate(1000, sum(exp(simulate(fit_allometry2_spaMM,
                                          newdata = data.frame(PA_area = data_rangers_clean_noG$PA_area[!is.na(data_rangers_clean_noG$staff_rangers)]),
...
```{r simu4, cache=TRUE}
set.seed(1)
simu4 <- replicate(1000, sum(exp(simulate(fit_allometry2_spaMM, newdata = data.frame(PA_area = PA_area_for_pred),
                                          type = "predVar", variances = list(linPred = TRUE, disp = TRUE),
                                          verbose = c(type = FALSE))) - 1))
...
Note to self: why are the result so different... Need to check what uncertainty is carried on with `simulate.HLfit`.

#### Using a GLM

We now try to use an heteroscedastic Generalised Linear Model (GLM) with a Gaussian family and a log link.
This is similar to the LM approach from above, except that the residuals of such a model are computed on the original scale instead of being computed on the log scale. This model does not fit the data very well (despite the consideration of heteroscedasticity which does improves the situation a little), so I would not trust its predictions very much. Yet it is useful to see how sensitive our predictions are with respect to this particular modelling assumption.

As before, we start by predicting the number of rangers for the countries for which we know it to check that things are OK:

```{r glm}
fit_allometry3_spaMM <- fitme(I(staff_rangers + 1) ~  log(PA_area) - 1,
                              resid.model = ~ log(PA_area),
                              data = data_rangers_clean_noG, family = gaussian(link = "log"))
```

```{r simu5, cache=TRUE}
set.seed(1)
simu5 <- replicate(1000, sum(simulate(fit_allometry3_spaMM,
                                          newdata = data.frame(PA_area = data_rangers_clean_noG$PA_area[!is.na(data_rangers_clean_noG$staff_rangers)]),
                                          type = "predVar", variances = list(linPred = TRUE, disp = TRUE),
                                          verbose = c(type = FALSE)) - 1))
...
```{r simu6, cache=TRUE}
set.seed(1)
simu6 <- replicate(1000, sum(simulate(fit_allometry3_spaMM, newdata = data.frame(PA_area = PA_area_for_pred),
                                          type = "predVar", variances = list(linPred = TRUE, disp = TRUE),
                                          verbose = c(type = FALSE)) - 1))
